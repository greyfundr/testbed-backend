...
import { NotificationPreferences } from '../../settings/interface/settings.interface';
...
export class NotificationService {
    ...
    async notify(
        ...
        const settings = await this.settingsService.getSettings(userUuid);
        if (!settings) return;
        ...
        const user = { uuid: userUuid } as any; // Minimal user object for relation
        ...
        if (prefs.push && metadata?.pushToken) {
            await this.firebaseService.sendPushNotification(metadata.pushToken, title, message, metadata);
        }
    ...
    async getUserNotifications(userUuid: string) {
