...
export class AdminEntity1772385207051 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`campaigns\` DROP COLUMN \`fee_percentage\``,
    );
    await queryRunner.query(
      `ALTER TABLE \`campaigns\` ADD \`fee_percentage\` decimal(5,2) NOT NULL DEFAULT '0.00'`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`campaigns\` ADD \`fee_percentage\` bigint NOT NULL DEFAULT '0'`,
    );
    await queryRunner.query(
